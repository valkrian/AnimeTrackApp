...
async def get_seasonal_anime(
    year: Optional[int] = Query(
        None, description="Year for specific season (e.g., 2024)"
    ),
    season: Optional[str] = Query(
        None, description="Season name (winter, spring, summer, fall)"
    ),
    ...
    try:
        ...
        if year and (year < 1900 or year > 2030):
            raise HTTPException(
                status_code=400, detail="Invalid year. Must be between 1900 and 2030"
            )
