...
class InMemoryCache:
    ...
    def set(
        self, key: str, data: SeasonalAnimeResponse, ttl_minutes: Optional[int] = None
    ) -> None:
        """
        ...
        entry = CacheEntry(
            data=data, timestamp=now, expires_at=expires_at, cache_key=key
        )
...
async def get_seasonal_anime_cache(
    year: Optional[int] = None, season: Optional[str] = None
) -> Optional[SeasonalAnimeResponse]:
    """
...
async def set_seasonal_anime_cache(
    data: SeasonalAnimeResponse,
